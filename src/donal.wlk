...
object donal {
	...
	method cantidadDolar() {
		if ((!game.hasVisual(signoPeso)) and (self.dinero() < -99)) {
			...
			if ((game.hasVisual(signoPeso)) and (self.dinero() > -100)) {
				game.removeVisual(signoPeso)
			}
		}
		cifra0.desaparecer()
		cifra1.desaparecer()
	...
	method cantidadVida() {
		if (elixir == 0) {
			if (game.hasVisual(elixir1)) {
				game.removeVisual(elixir1)
			}
			if (game.hasVisual(elixir2)) {
				game.removeVisual(elixir2)
			}
			if (game.hasVisual(elixir3)) {
				game.removeVisual(elixir3)
			}
			if (game.hasVisual(elixir4)) {
				game.removeVisual(elixir4)
			}
		}
		if (vida == 3) {
			if (!game.hasVisual(vida1)) {
				vida1.aparecer()
			}
			if (!game.hasVisual(vida2)) {
				vida2.aparecer()
			}
			if (!game.hasVisual(vida3)) {
				vida3.aparecer()
			}
			...
			if (vida == 2) {
				...
				if (elixir == 1) {
					game.addVisual(elixir3)
				}
				if (elixir == 2) {
					game.addVisual(elixir4)
				}
			} else {
				if (vida == 1) {
					if (game.hasVisual(elixir3)) {
					...
					if (elixir == 1) {
						game.addVisual(elixir1)
					}
					if (elixir == 2) {
						game.addVisual(elixir2)
					}
				}
				if ((vida == 1) and (game.hasVisual(vida2))) {
					vida2.desaparecer()
				} else {
					if (vida == 0) {
						vida1.desaparecer()
	...
	method quitarVida(algo) {
		...
		if (algo == 'putin') {
			...
			game.say(algo, "PERDISTE UNA VIDA, CUIDADO")
			coronavirus.mover()
		}
		if (vida <= 0) {
